...
static int recreate_format_l(JNIEnv *env, IJKFF_Pipenode *node, VideoClip *player)
{
    IJKFF_Pipenode_Opaque *opaque         = node->opaque;
    ...
    rotate_degrees = ffp_get_video_rotate_degrees(ffp, player);
    ...
    SDL_AMediaCodecBufferInfo *buffer_info)
{
    IJKFF_Pipenode_Opaque *opaque     = node->opaque;
    FFPlayer              *ffp        = opaque->ffp;
    VideoState            *is         = opaque->player->is_0x30;
...
static int feed_input_buffer2(JNIEnv *env, IJKFF_Pipenode *node, int64_t timeUs, int *enqueue_count)
{
    ...
    if (!d->packet_pending || d->queue->serial != d->pkt_serial) {
        ...
        do {
            ...
            if (ffp_packet_queue_get_or_buffering(ffp, opaque->player, d->queue, &pkt, &d->pkt_serial, &d->finished, 0) < 0) {
                ret = -1;
                goto fail;
            }
        ...
        if (ffpipeline_is_surface_need_reconfigure_l(pipeline)) {
            ...
            if (!opaque->aformat_need_recreate &&
                ...
                if (opaque->aformat_need_recreate) {
                    ...
                    ret = recreate_format_l(env, node, opaque->player);
                    if (ret) {
...
static int enqueue_thread_func(void *arg, void *arg2)
{
    JNIEnv                *env      = NULL;
...
static int drain_output_buffer2(JNIEnv *env, IJKFF_Pipenode *node, int64_t timeUs, int *dequeue_count, AVFrame *frame, AVRational frame_rate)
{
    ...
    if (got_frame) {
        ...
        ret = ffp_queue_picture2(ffp, opaque->player, frame, pts, duration, av_frame_get_pkt_pos(frame), is->viddec.pkt_serial);
        if (ret) {
            if (frame->opaque)
                SDL_VoutAndroid_releaseBufferProxyP(opaque->weak_vout, (SDL_AMediaCodecBufferProxy **)&frame->opaque, false);
        }
...
static int func_run_sync_loop(IJKFF_Pipenode *node) {
    ...
    if (!opaque->acodec) {
        return ffp_video_thread(ffp, opaque->player);
    }
...
//    }
    ...
    opaque->enqueue_thread = SDL_CreateThreadEx(&opaque->_enqueue_thread, enqueue_thread_func, node, opaque->player, "amediacodec_input_thread");
...
int ffpipenode_config_from_android_mediacodec(FFPlayer *ffp, IJKFF_Pipeline *pipeline, SDL_Vout *vout, IJKFF_Pipenode *node) {
    ...
    ffp_set_video_codec_info(ffp, opaque->player, MEDIACODEC_MODULE_NAME, opaque->mcc.codec_name, opaque->mcc.mime_type);
...
IJKFF_Pipenode *ffpipenode_create_video_decoder_from_android_mediacodec(FFPlayer *ffp, VideoClip *player, IJKFF_Pipeline *pipeline, SDL_Vout *vout)
{
    ALOGD("ffpipenode_create_video_decoder_from_android_mediacodec()\n");
    if (SDL_Android_GetApiLevel() < IJK_API_16_JELLY_BEAN)
        return NULL;

    if (!ffp || !player || !player->is_0x30)
        return NULL;
    ...
    ret = recreate_format_l(env, node, player);
    if (ret) {
        ALOGE("amc: recreate_format_l failed\n");
        goto fail;
    }
    ...
    ffp_set_video_codec_info(ffp, player, MEDIACODEC_MODULE_NAME, opaque->mcc.codec_name, opaque->mcc.mime_type);
