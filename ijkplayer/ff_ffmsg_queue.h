...
typedef struct MessageQueue {
    AVMessage *first_msg, *last_msg; //0x1d0, 0x1d8
    int nb_messages; //0x1e0
    int abort_request; //0x1e4
    SDL_mutex *mutex; //0x1e8
    SDL_cond *cond; //0x1f0

    AVMessage *recycle_msg; //0x1f8
    int recycle_count; //0x1fc
    int alloc_count; //0x200
} MessageQueue;
...
{
    int ret;
    ALOGD("msg_queue_put: %d, %d, %d", msg->what, msg->arg1, msg->arg2);
